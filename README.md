...
Cost-tiered subagents. Seven agents, each pinning its own model and reasoning
...
| `chore` | opus, low | 2.6× | code whose shape is already decided |
| `dev` | opus, medium | 6.4× | code that needs correctness reasoning |
| `critic` | opus, high | 8.7× | ordinary review, verify a claim — the default thinker |
| `sage` | opus, xhigh | 16.5× | architecture, auth/money/migration diffs, or `critic` failed |
| `oracle` | opus, max | 28.5× | irreversible calls, or `sage` failed |

Cost is measured $/task-run relative to haiku. **The agent's own ceiling is
`dev` for code and `critic` for judgement**; `sage` and `oracle` are dispatched
only when the user asks for them. Failure above that line is reported with a
proposed tier and its cost, not acted on.

Three findings do most of the work. **Every rung above haiku is Opus 5.5** — on
the 2026-09 Artificial Analysis index, every Fable 5.1 and Sonnet 5 point is
beaten by an opus point that is cheaper and scores at least as high (fable-high
costs 3× opus-medium for the same 51). 2.0.0 therefore moved `critic`, `sage`
and `oracle` off fable and **retired `judge`**: with the think side on opus
there is no hull point left between `sage` (xhigh) and `oracle` (max). And the
**marginal cost per index point jumps 5.1× at `critic` → `sage`** ($0.16/pt →
$0.82/pt) — right on the autonomous ceiling, which is why escalation past it is
triggered by observed failure and granted by the user: below that cliff guessing
high costs pennies, above it guessing high is the main way to waste money.
...
and `.claude-plugin/`. In a new session the seven agents show up in the Agent
...
plugins/model-routing/
  ...
  agents/*.md                        7 agents (auto-discovered)
...
gives you seven duplicate agent names and the policy injected twice per session.
